...
#Iperf server
tcp_server = sta1.popen(f'iperf3 -s -p {TCP_PORT} --logfile udp_server.txt')

#Iperf client
tcp_client = sta2.popen(f'iperf3 -c 10.0.0.1 -t {TCP_PORT} -u -b 50M -i 1 -t 0 > --logfile iperf.txt')
...
def get_latest_bitrate(filename="udp_server.txt"):
    try:
        ...
        for line in reversed(lines):
            if "Gbits/sec" in line:
                ...
                value = parts[-6]
                value1 = float(value)
                #print(f"Found throughput: {value1} Gbits/sec")
                value2 = value1*1000
                return float(value2)
...
def get_latest_jitter(filename="udp_server.txt"):
	try:
		with open(filename, "r") as f:
			lines = f.readlines()
		...
		for line in reversed(lines):
			if "bits/sec" in line:
				...
				value = parts[-4]
				value1 = float(value)
				#print(f"Found jitter: {value1} ms")
				return float(value1)
	except (FileNotFoundError, ValueError, IndexError) as e:
		print(f"Error reading throughput: {e}")
		return 0
...
log_file.write("time,sta1_y,y_diff,rssi1,throughput,rtt,estimated latency,jitter\n")
...
# -----------------------------
print("*** UDP trial with STA moving")
...
#Wait for iperf and other processes to load
time.sleep(10)
...
while y1 <= STOP_POSITION:
	...
	rtt = get_latest_ping()
	est_latency = rtt/2
	jitter = get_latest_jitter()
	current_time = time.time() - start_time
	log_file.write(f"{current_time},{y1},{y_diff},{rssi_1},{iperf_bitrate},{rtt},{est_latency},{jitter}\n")
	log_file.flush()
...
fig.suptitle("UDP Metrics")
ax.plot(y_diff,throughput)
ax.set_title('y_diff vs throughput, UDP')
ax.set_xlabel("Distance from STAs, (m)")
ax.set_ylabel("Throughput, (MBits/sec)")
ax = plt.subplot(2,2,2)
ax.plot(time,rssi)
ax.set_title('rssi over time, UDP')
ax.set_xlabel("time, (s)")
ax.set_ylabel("rssi, (dBm)")
ax = plt.subplot(2,2,3)
ax.plot(time,latency)
ax.set_title('latency over time, UDP')
ax.set_xlabel("time, (s)")
ax.set_ylabel("latency, (s)")
ax = plt.subplot(2,2,4)
ax.plot(time,rtt)
ax.set_title('Rtt, UDP')
ax.set_xlabel("time, (s)")
ax.set_ylabel("Rtt, (s)")
plt.show()
print("*** UDP trial complete\n")
info("*** Running CLI\n")
CLI(net)
