...
#Iperf client
tcp_client = sta2.popen(f'iperf3 -c 10.0.0.1 -t {TCP_PORT} -u -b 15M -i 1 -t 0 > --logfile iperf.txt')
    ...
    try:
        ...
        for line in reversed(lines):
            ...
            if "time" in line.lower():
                ...
                if match:
                    value = float(match.group(1)) # Extracts just the number (e.g., '14.2')
                    #print(value)
                    value = value/2
                    return float(value)
...
while y1 <= STOP_POSITION:
	...
	position_in_cycle = total_traveled % cycle_length
	if position_in_cycle < forward_distance:
		y1_change = step_size
		y2_change = -step_size
	else:
		y1_change = -step_size
